...
type AccountTotals = Pick<
	TransactionsAccount,
	'savings' | 'investments' | 'foreignCurrency'
>;
...
const PEPPER_LOGGED_IN_SELECTORS: readonly string[] = [
	...
	ua(
		`resourceIdMatches("${PACKAGE_NAME}:id/.*[Bb]alance.*").className("android.widget.TextView")`,
	),
...
const PEPPER_BALANCE_SELECTORS: readonly string[] = [
	...
	ua(
		'resourceIdMatches("(?i).*balance.*").className("android.widget.TextView")',
	),
	ua(
		`resourceIdMatches("${PACKAGE_NAME}:id/.*[Bb]alance.*").className("android.widget.TextView")`,
	),
...
const PEPPER_PASSWORD_SELECTORS: readonly string[] = [
	...
	"//android.widget.EditText[contains(@resource-id,'Password')]",
	ua(
		'resourceIdMatches("(?i).*password.*").className("android.widget.EditText")',
	),
...
export function normalizePepperAccountNumber(raw: string): string {
	...
	if (labeled) {
		const chunk = stripBidiAndTrim(labeled[1])
			.replace(/\s+/g, '')
			.replace(/•/g, '*');
...
export function parseCurrencyAmountSnippet(
	raw: string,
	...
	if (ilsTrailing) {
		return {
			amount: parseFloat(ilsTrailing[1].replace(/,/g, '')),
			currency: 'ILS',
		};
	}
	...
	if (usdTrailing) {
		return {
			amount: parseFloat(usdTrailing[1].replace(/,/g, '')),
			currency: 'USD',
		};
	}
	...
	const isoLead =
		/\b(USD|EUR|GBP|CHF|JPY|CAD|AUD|PLN)\b\s*[:\s]*([\d,.\-+]+)/gi;
...
export function extractForeignCurrencyAmountsFromText(
	raw: string,
): CurrencyAmount[] {
	const t = stripBidiAndTrim(raw);
	if (!t) {
...
export function firstIlsAmountFromCompoundText(
	raw: string,
): number | undefined {
	const t = stripBidiAndTrim(raw);
	const re = /(?:₪|\u20aa)\s*([\d,.\-+]+)/g;
...
function mergeForeignAmounts(
	gather: CurrencyAmount[],
	next: CurrencyAmount[],
): void {
...
export default class PepperScraper extends BaseAndroidAppScraper<PepperCredentials> {
	...
	private async readBalanceFromShekelElementsScan(): Promise<
		number | undefined
	> {
		...
		try {
			...
			debug(
				'readBalanceFromShekelElementsScan failed: %s',
	...
	private async readBalanceFromWalkAncestorsFromLabels(): Promise<
		number | undefined
	> {
	...
	private async ensurePepperSessionUiReady(): Promise<void> {
		...
		if (await this.isPepperLoginScreen()) {
			...
			if (
				await this.isAnyVisible(PEPPER_HOME_DASHBOARD_READY_SELECTORS, 8_000)
			) {
		...
		if (await this.isAnyVisible(PEPPER_PROFILE_SCREEN_SELECTORS, 1_500)) {
			...
			await sleep(900);
			if (
				await this.isAnyVisible(PEPPER_HOME_DASHBOARD_READY_SELECTORS, 3_000)
			) {
				return;
			}
		}
		...
		debug(
			'Restoring Pepper UI to home (session snapshot may have opened off-dashboard)',
		);

		for (let attempt = 0; attempt < 6; attempt++) {
			if (
				await this.isAnyVisible(PEPPER_HOME_DASHBOARD_READY_SELECTORS, 1_400)
			) {
				return;
			}
	...
	private async dismissWelcomeIfPresent(): Promise<void> {
		const hit = await this.isAnyVisible(
	...
	private async enterPhoneDigitsViaKeypad(digits: string): Promise<void> {
		...
		for (const ch of digits) {
			await this.tapAny(
				[
					`//*[@text="${ch}" and (@clickable="true" or @focusable="true")]`,
					ua(`text("${ch}").clickable(true)`),
				],
				14_000,
			);
	...
	private async ensurePepperForegroundClosingForeignApps(
		maxPresses = 4,
	): Promise<void> {
	...
	private async isLoginChromeVisible(): Promise<boolean> {
		...
		return this.isAnyVisible(
			['//*[@text="כניסה לחשבון"]', ua('textContains("כניסה לחשבון")')],
			450,
		);
	}

	private async pollLoggedInOrOtpAfterPassword(
		totalMs: number,
	): Promise<'logged_in' | 'otp'> {
		...
		throw new TimeoutError(
			`Timed out after ${totalMs}ms waiting for Pepper home screen or SMS/code verification UI`,
		);
	}

	private async tapPostCredentialsSubmitOptional(
		visibleBudgetMs = 2_800,
	): Promise<void> {
		const hit = await this.isAnyVisible(
	...
	private async tapVerifyOptional(): Promise<void> {
		...
		if (!hit) {
			debug(
				'No OTP verify button found; assuming submit happens automatically',
			);
			return;
		}
	...
	private phoneDigitsMatch(
		existingRaw: string,
		wantDialDigits: string,
	): boolean {
	...
	private async enterPhoneStep(credentials: PepperCredentials): Promise<void> {
		...
		if (await this.isAnyVisible(PEPPER_PASSWORD_SELECTORS, 4_500)) {
			...
			const phoneAlsoVisible = await this.isAnyVisible(
				PEPPER_PHONE_SELECTORS_STRICT,
				1_500,
			);
			if (!phoneAlsoVisible) {
		...
		const strictVisible = await this.isAnyVisible(
			PEPPER_PHONE_SELECTORS_STRICT,
			11_000,
		);
		if (strictVisible) {
	...
	private async enterPepperOtpCode(code: string): Promise<void> {
		...
		for (const ch of code) {
			...
			if (n < 0 || n > 9) {
				throw new Error(
					`OTP contains non-digit character: ${JSON.stringify(ch)}`,
				);
			}
		}
		...
		if (await this.isAnyVisible(PEPPER_PHONE_SELECTORS_STRICT, 600)) {
			throw new Error(
				'After OTP entry we are back on the login screen — OTP entry was routed to the wrong field',
			);
		}
	}
	...
	private async dismissNotificationPopupIfPresent(): Promise<void> {
		if (
			!(await this.isAnyVisible(PEPPER_NOTIFICATION_POPUP_SELECTORS, 2_500))
		) {
			return;
		}
	...
	private async acceptTermsIfPresent(timeoutMs = 8_000): Promise<void> {
		...
		const checkboxVisible = await this.isAnyVisible(
	...
	async login(credentials: PepperCredentials): Promise<ScraperLoginResult> {
		...
		if (!loginFormVisible) {
			...
			if (await this.isAnyVisible(PEPPER_TERMS_SCREEN_SELECTORS, 3_000)) {
				...
				debug(
					'Terms page showing at session start; accepting and waiting for home',
				);
				await this.acceptTermsIfPresent(12_000);
		...
		const passwordAlreadyVisible = await this.isAnyVisible(
		...
		if (!credentials.otpCodeRetriever) {
			return {
				...
				errorMessage:
					'Pepper is asking for SMS/code verification. Provide otpCodeRetriever to supply the code.',
			};
		}
	...
	private async dashboardIlsNearLabelWalk(
		labelNeedle: string,
		...
		try {
			...
			debug(
				'dashboardIlsNearLabelWalk(%s) failed: %s',
				labelNeedle,
				e instanceof Error ? e.message : String(e),
			);
			return undefined;
	...
	private async dashboardIlsAfterLabelNth(
		labelNeedle: string,
		nth: number,
	): Promise<number | undefined> {
		try {
	...
	private async dashboardForeignBalances(): Promise<
		CurrencyAmount[] | undefined
	> {
		const pivotXpaths = [
		...
		for (const px of [...pivotXpaths, ...pivotUi]) {
			try {
				...
				for (let depth = 0; depth < 5; depth++) {
					...
					mergeForeignAmounts(
						gather,
						extractForeignCurrencyAmountsFromText(blob),
					);
					if (gather.length >= 6) {
						break;
					}
				...
				try {
					const followingEls = await anchor
						.$$('xpath=following::android.widget.TextView')
						.getElements();
					for (let i = 0; i < Math.min(28, followingEls.length); i++) {
						...
						mergeForeignAmounts(
							gather,
							extractForeignCurrencyAmountsFromText(raw),
						);
						if (gather.length >= 6) {
							break;
						}
	...
	private async dashboardForeignBalancesBroadScan(): Promise<
		CurrencyAmount[] | undefined
	> {
		const fxHint =
			/[$€£¥]|מט\u05f4ח|מט["׳']ח|\bUSD\b|\bEUR\b|\bGBP\b|\bCHF\b|\bJPY\b|\bCAD\b|\bAUD\b|\bPLN\b/i;
		try {
			const size = await this.driver
				.getWindowSize()
				.catch(() => ({ height: 2400 }));
			...
			debug(
				'dashboardForeignBalancesBroadScan failed: %s',
				e instanceof Error ? e.message : String(e),
			);
			return undefined;
		}
	...
	private async readPepperAccountTotals(): Promise<AccountTotals | undefined> {
		...
		try {
			const savings =
				(await this.dashboardIlsNearLabelWalk('חסכונות')) ??
				(await this.dashboardIlsAfterLabelNth('חסכונות', 1));
	...
	private async readAccountNumberFromProfileClipboard(): Promise<
		string | undefined
	> {
		await this.ensurePepperHomeDashboard();
		try {
		...
		if (
			digits.length >= 6 &&
			digits.length <= 14 &&
			this.isLikelyPepperAccountToken(digits)
		) {
			return digits;
		}
		return undefined;
	...
	async probePepperDashboard(): Promise<PepperDashboardProbeResult> {
		...
		const sels: { name: string; sel: string }[] = [
			...
			{
				name: 'accountTextMarker',
				sel: '//*[contains(@text,"חשבון") or contains(@content-desc,"חשבון")]',
			},
		];
		...
		for (const { name, sel } of sels) {
			try {
				...
				if (exists) {
					...
					if (displayed) {
						accessibleTextPreview = (await this.readAccessibleText(el)).slice(
							0,
							240,
						);
					}
		...
		try {
			...
			for (const el of shekelEls) {
				try {
					...
					const loc = await el.getLocation().catch(() => ({ x: 0, y: 0 }));
					const accessibleText = (await this.readAccessibleText(el)).slice(
						0,
						200,
					);
		...
		await tryPath('readBalanceFromWalkAncestorsFromLabels', () =>
			this.readBalanceFromWalkAncestorsFromLabels(),
		);
		await tryPath('dashboardIlsAfterLabelNth_זאת_היתרה', () =>
			this.dashboardIlsAfterLabelNth('זאת היתרה', 1),
		);
		await tryPath('readBalanceFromHeroCompound', () =>
			this.readBalanceFromHeroCompound(),
		);
		await tryPath('readBalanceFromShekelElementsScan', () =>
			this.readBalanceFromShekelElementsScan(),
		);
		...
		return {
			...
			foreignBroadScanPreview: foreignBroad
				?.slice(0, 8)
				.map((c) => `${c.currency}:${c.amount}`),
		};
	}

	async runProbeSession(
		credentials: PepperCredentials,
	): Promise<PepperDashboardProbeResult> {
	...
	async fetchData(): Promise<ScraperScrapingResult> {
		...
		try {
			...
			debug(
				'readPepperAccountTotals failed: %s',
				e instanceof Error ? e.message : String(e),
			);
		}
		...
		try {
			...
			debug(
				'readAccountNumber failed: %s',
				e instanceof Error ? e.message : String(e),
			);
		}

		const safeAccountNumber =
			accountNumber.length > 0 ? accountNumber : 'unknown';
