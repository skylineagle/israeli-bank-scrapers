...
Some institutions have no usable web interface, so they are scraped by driving the native Android app
inside an emulator via [Appium](https://appium.io/) and the UiAutomator2 driver.

`BaseAndroidAppScraper` owns all generic plumbing: emulator boot, Appium session lifecycle, RAM
snapshots, and shared element/interaction helpers. Concrete scrapers (e.g. [Pepper](./pepper/README.md))
only implement `appPackage`, `login()`, and `fetchData()`.
...
3. **An Android Virtual Device (AVD)**.
   ...
   - The scraper boots the first AVD found unless you pass `avdName`.
4. **Appium 2 + the UiAutomator2 driver**:
   ```bash
   bun add -g appium
   appium driver install uiautomator2
   ```
   The base class auto-starts an Appium server on port `4723` if one is not already running.
5. **The target app installed in the AVD.** Install it once from the Play Store (or sideload an APK)
   inside the emulator. The base class resolves the launcher activity automatically; it does **not**
   install the APK for you.
6. **A baseline snapshot** named `scraper-baseline` (see below).

## Snapshots: how a run boots

To skip slow boot and (after the first login) repeat OTP flows, the base class uses emulator RAM
snapshots:

- **`scraper-baseline`** — a logged-out, booted state you create once. Boot your AVD, install/open
  the app to a stable pre-login screen, then save it:
  ```bash
...
4. **Reuse base helpers** instead of re-implementing them:
   - Waiting/visibility: `waitForAnyDisplayed`, `isAnyVisible`, `isVisible`, `waitForCondition`.
   - Interaction: `tapAny`, `typeIntoAny`, `tapByCoordinates`, `pressDigitsViaDriverKeyCode`,
     `pressDigitsViaAdbKeyevent`, `pressAndroidBack`, `dismissKeyboard`, `swipeUp`.
   - Reading: `readAccessibleText`, `readText`, `readAllTexts`, `readAndroidClipboardPlaintext`.
   - Process/foreground: `readForegroundPackage`, `bringTargetAppToForeground`, `spawnAdb`.
